...
void GameClear::Draw()
{
	...
	int width2 = GetDrawFormatStringWidthToHandle(m_pressFontHandle, "PRESS RETRY( X ) ");
	if ((m_frame / 30) % 2 == 0)
	{
		DrawStringToHandle((kScreenWidth / 2) - (width2 / 2), kScreenHeight / 2 + 128,
			"PRESS RETRY( X )", GetColor(255, 255, 0), m_pressFontHandle);
	}
...
{
	if (Pad::IsTrigger(PAD_INPUT_2)&&m_count>=100)
	{
		if (m_buttomNum == 0)
		{
			m_buttomNum++;
